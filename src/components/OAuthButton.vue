...
<template>
  <div data-auth-button-container class="relative">
    ...
    <button
      ...
      class="relative w-10 h-10 p-0 rounded-lg overflow-hidden border-3 border-[#3c3c3c] hover:border-[#ff66aa] transition-all hover:shadow-lg hover:shadow-[#ff66aa]/20 hover:scale-110 hover:cursor-pointer flex items-center justify-center bg-[#2e3038]"
    >
      <img
    ...
    >
      <div
        ...
        class="absolute right-0 mt-3 w-56 bg-[#22242a]/95 backdrop-blur-md border border-[#3c3c3c] rounded-sm shadow-2xl shadow-black/40 overflow-hidden z-50"
      >
        <!-- User Info Section -->
          ...
          <div class="space-y-1">
            ...
            <div v-if="userData?.statistics_rulesets?.osu" class="pt-2 space-y-1.5">
              <div class="flex justify-between items-center text-xs">
                ...
                <span class="font-medium text-[#ff66aa]">
                  #{{ formatNumber(userData.statistics_rulesets.osu.global_rank) }}
                </span>
              </div>
              <div class="flex justify-between items-center text-xs">
                <span class="text-[#666666]">表现值</span>
                <span class="font-medium text-[#ff66aa]">
                  {{ formatNumber(userData.statistics_rulesets.osu.pp) }} PP
                </span>
              </div>
            </div>
          </div>
        ...
        <div class="p-2">
          <button
            @click="handleLogout"
            class="w-full px-4 py-2.5 text-sm font-medium text-[#999999]  bg-[#2e3038]/40 hover:bg-[#ff66aa] hover:text-[#1a1b1e] rounded-lg transition-all duration-150 ease-out"
          >
            登出
          </button>
        </div>
